import os
import time

from openai import OpenAI

from src.extractors.base import BaseExtractor, ExtractionResult
from src.utils.config import OPENAI_MODEL, MAX_TOKENS, REQUEST_TIMEOUT, EXTRACTION_PROMPT
from src.utils.image_loader import load_image_b64


class ChatGPTExtractor(BaseExtractor):
    def __init__(self):
        # Read key at instantiation time so Docker env vars are always current
        api_key = os.environ.get("OPENAI_API_KEY", "")
        self.client = OpenAI(api_key=api_key, timeout=float(REQUEST_TIMEOUT))
        self.model = OPENAI_MODEL

    def extract(self, image_path: str) -> ExtractionResult:
        b64, mime_type = load_image_b64(image_path)
        start = time.monotonic()
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                max_tokens=MAX_TOKENS,
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {
                                "type": "image_url",
                                "image_url": {"url": f"data:{mime_type};base64,{b64}", "detail": "high"},
                            },
                            {"type": "text", "text": EXTRACTION_PROMPT},
                        ],
                    }
                ],
            )
            elapsed_ms = (time.monotonic() - start) * 1000
            text = response.choices[0].message.content or ""
            usage = response.usage
            return ExtractionResult(
                api_name="ChatGPT",
                model_id=self.model,
                image_path=image_path,
                raw_text=text.strip(),
                processing_time_ms=round(elapsed_ms, 2),
                prompt_tokens=usage.prompt_tokens,
                completion_tokens=usage.completion_tokens,
                total_tokens=usage.total_tokens,
            )
        except Exception as exc:
            elapsed_ms = (time.monotonic() - start) * 1000
            return ExtractionResult(
                api_name="ChatGPT",
                model_id=self.model,
                image_path=image_path,
                raw_text="",
                processing_time_ms=round(elapsed_ms, 2),
                prompt_tokens=0,
                completion_tokens=0,
                total_tokens=0,
                error=str(exc),
            )
